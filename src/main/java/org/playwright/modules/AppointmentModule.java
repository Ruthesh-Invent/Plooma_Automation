	    	...
	    	getAppointmentPage().saveAppointmentHandleAllPopups();
	          ...
	          if (afterCount > beforeCount) {
	        	    extentTest.get().log(Status.PASS, 
	        	        "Appointment added successfully. Count increased from " 
	        	        + beforeCount + " to " + afterCount);
	        	} else {
	        	    extentTest.get().log(Status.FAIL, 
	        	        "Appointment was NOT added! Count did NOT increase. Before: " 
	        	 ...
	        	 getDashboardPage().navigateToTile("Checked In");
		          ...
		          getDashboardPage().selectgoalandobj();
		           ...
		           getPage().waitForURL(url -> url.contains("superbill"), 
		            ...
		            getDashboardPage().selectICDcode("Procedure Codes","92507 - Treatment of speech, language, voice, communication, and/or auditory processing disorder; individual");
		            getDashboardPage().clicksumbit();
		            getPage().waitForLoadState();
		            getPage().waitForURL(url -> url.contains("/dashboard/main"), new WaitForURLOptions()
		                    .setTimeout(60000)
		                    .setWaitUntil(WaitUntilState.LOAD));
